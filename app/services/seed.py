...
## Membership
Membership is open to all students. Anyone can register and log in \
immediately — no approval is required to create an account.

## Editorial Review
Every piece of writing a member submits — poetry, shayari, blogs, articles, \
reviews, or other literary work — is reviewed by an Editor before it becomes \
publicly visible. Editors approve or reject submissions and may share \
feedback; rejected work can be revised and resubmitted.
...
5. Reports are reviewed by Editors and Admins; repeated violations may \
   result in restricted posting privileges.
...
def run_seed(admin_email=None, admin_password=None, editor_email=None, editor_password=None):
    ...
    if admin_email:
        ...
        if user:
            ...
            base_username = re.sub(r"[^a-z0-9]+", "-", admin_email.split("@")[0].lower()).strip("-") or "admin"
            ...
            user = User(
                name="Society Admin", email=admin_email, username=username,
                role=UserRole.ADMIN.value, status=UserStatus.APPROVED.value,
            )
            user.set_password(admin_password)
            db.session.add(user)
        if not user.username:
            user.username = admin_email.split("@")[0].lower()
        db.session.commit()

    if editor_email:
        user = User.query.filter_by(email=editor_email).first()
        if user:
            ...
            if not editor_password:
                raise ValueError("editor_password is required when creating a new editor user")
            base_username = re.sub(r"[^a-z0-9]+", "-", editor_email.split("@")[0].lower()).strip("-") or "editor"
            username = base_username
            n = 1
            while User.query.filter_by(username=username).first() is not None:
                n += 1
                username = f"{base_username}-{n}"
            user = User(
                name="Society Editor", email=editor_email, username=username,
                role=UserRole.EDITOR.value, status=UserStatus.APPROVED.value,
            )
