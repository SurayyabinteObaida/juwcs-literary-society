...
def register_cli(app):
    ...
    @click.option("--admin-email", default=lambda: os.environ.get("ADMIN_EMAIL"),
                  help="Create/promote an admin with this email. Falls back to the ADMIN_EMAIL env var.")
    @click.option("--admin-password", default=lambda: os.environ.get("ADMIN_PASSWORD"),
                  help="Password for the admin account (required if creating). Falls back to ADMIN_PASSWORD.")
    @click.option("--editor-email", default=lambda: os.environ.get("EDITOR_EMAIL"),
                  help="Create/promote an editor with this email. Falls back to the EDITOR_EMAIL env var.")
    @click.option("--editor-password", default=lambda: os.environ.get("EDITOR_PASSWORD"),
                  help="Password for the editor account (required if creating). Falls back to EDITOR_PASSWORD.")
    def seed(admin_email, admin_password, editor_email, editor_password):
        ...
        Safe to run on every deploy: community/badge seeding is idempotent
        (checked by slug/label before insert), and the admin/editor block
        only runs at all when an email is actually supplied (via flag or the
        ADMIN_EMAIL/EDITOR_EMAIL env vars) — if the user already exists it
        is promoted in place rather than duplicated, and its password is
        left untouched.
        """
        ...
        try:
            ...
            # A missing *_PASSWORD env var when the account doesn't exist yet
            # is a config problem, not a reason to take the whole app down —
            # this command runs as part of the Render start command, ahead
            # of gunicorn. Log it loudly and let the app boot regardless;
            # community/badge seeding above this point already committed.
            click.echo(f"Seed warning: {exc} (community/badge seed still completed)", err=True)
            return
