...
def create():
    ...
    if form.validate_on_submit():
        ...
        notification_service.notify_bethak_opened(session)
...
def reopen(session_id):
    session = BethakSession.query.get_or_404(session_id)
    was_closed = session.status == BethakStatus.CLOSED.value
    session.status = BethakStatus.OPEN.value
    db.session.commit()
    flash("Bethak reopened.", "info")
    # Only notify on an actual CLOSED -> OPEN transition, never a no-op.
    if was_closed:
        notification_service.notify_bethak_opened(session)
    return redirect(url_for("bethak.view", session_id=session_id))
